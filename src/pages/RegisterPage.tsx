...
type Step = 'methods' | 'email' | 'code' | 'details';
...
const RegisterPage = () => {
  ...
  const { register, isAuthenticated, startTelegramAuth, cancelTelegramAuth, telegramAuthStatus } = useAuth();
  ...
  const handleVerifyCode = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await authApi.verifyCode(email, code, 'register');
      setStep('details');
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Ошибка подключения');
    } finally {
  ...
  const goBack = () => {
    ...
    else if (step === 'details') { setStep('code'); setPassword(''); setConfirmPassword(''); setUsername(''); }
    else if (step === 'email') { setStep('methods'); setEmail(''); }
  };

  const getFieldError = (field: string): string | null => fieldErrors[field]?.[0] || null;
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-white/50 text-sm mt-1">Присоединяйся к сообществу</p>
        </div>
        ...
        <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6 space-y-5">
          ...
          {step === 'details' && (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <p className="text-zinc-400 text-sm">Регистрация как <span className="text-white font-medium">{email}</span></p>
              ...
              <div>
                ...
                <input
                  ...
                  className={`${inputClass} ${getFieldError('username') ? '!border-red-500 !focus:ring-red-500' : ''}`}
                  autoFocus
                />
                {getFieldError('username') && <p className="text-red-400 text-xs mt-1">{getFieldError('username')}</p>}
                <p className="text-zinc-500 text-xs mt-1">3-20 символов, латиница, цифры и _</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`${inputClass} pr-12 ${getFieldError('password') ? '!border-red-500 !focus:ring-red-500' : ''}`}
                  />
                ...
                <p className="text-zinc-500 text-xs mt-1">Минимум 8 символов, 1 заглавная буква и 1 цифра</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-white/70 mb-1.5">Повторите пароль</label>
                <input
                  type={showPassword ? 'text' : 'password'}
              ...
              <button type="submit" disabled={isLoading} className={btnPrimary}>
                {isLoading ? <><Spinner /> Создание...</> : 'Создать аккаунт'}
              </button>
            </form>
          )}
        </div>
